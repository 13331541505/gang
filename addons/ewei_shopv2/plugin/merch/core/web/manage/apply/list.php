...
class List_EweiShopV2Page extends MerchWebPage
{
	protected function applyData($status, $st)
	{
		...
		$sql = 'select b.*,u.merchname,u.realname,u.mobile from ' . tablename('ewei_shop_merch_bill') . ' b ' . ' left join ' . tablename('ewei_shop_merch_user') . ' u on b.merchid = u.id' . (' where 1 ' . $condition . ' ORDER BY ' . $orderby . ' desc ');
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			foreach ($lists as $k => $v) {
				...
				// pdo_update('ewei_fen_log', array('status'=>1), array('id' => $v['logid']));
			}
