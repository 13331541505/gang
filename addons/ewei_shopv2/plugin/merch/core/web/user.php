...
class User_EweiShopV2Page extends PluginWebPage
{
	...
	protected function post()
	{
		...
		//查询游戏活动参数
		$gamesetting = pdo_fetch('select * from '.tablename('ewei_mch_money').' where in_id='.$id.' and type=1');
		if (empty($item)) {
			$item['iscredit'] = 1;
		...
		if ($_W['ispost']) {
			...
			if (empty($item)) {
				...
				$url = mobileUrl('shop/paycode', ['shop_id'=>$id], true);
	    		$qrcode = m('qrcode')->createQrcode($url);

				pdo_insert('ewei_shop_merch_account', $account);
				$accountid = pdo_insertid();
				pdo_update('ewei_shop_merch_user', array('accountid' => $accountid,'pay_code'=>$qrcode), array('id' => $id));
				...
				pdo_update('ewei_mch_money', $data1, array('in_id' => $id));
				if (!empty($item['accountid'])) {
