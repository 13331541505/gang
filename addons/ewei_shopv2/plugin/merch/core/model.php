...
class MerchModel extends PluginModel
{
	...
	public function allPerms()
	{
		if (empty(self::$allPerms)) {
			...
			$perms = array('shop' => $this->perm_shop(),  'order' => $this->perm_order(), 'statistics' => $this->perm_statistics(), 'perm' => $this->perm_perm(), 'apply' => $this->perm_apply(), 'quick' => $this->perm_quick());
			self::$allPerms = $perms;
		}
	...
	protected function perm_order()
	{
		return array(
			...
			// ),
			'list'      => array('text' => '订单管理', 'main' => '浏览全部订单', 'status_1' => '浏览关闭订单', 'status0' => '浏览待付款订单', 'status1' => '浏览已付款订单',  'status3' => '浏览完成的订单', 'status4' => '浏览退货申请订单'),
			'op'        => array(
				'text'          => '操作',
				'delete'        => '订单删除-log',
				'pay'           => '确认付款-log',
...
请到后台查看~', 'color' => '#000000')
	...
	public function getMerchPrice($merchid, $flag = 0)
	{
		...
		$orderids = array();
		$sql = 'select ' . $con . ' from ' . tablename('ewei_shop_merch_user') . ' u ' . ' left join ' . tablename('ewei_shop_order') . ' o on u.id=o.merchid left join '.tablename('ewei_fen_log').' l on l.orderid=o.id ' . (' where 1 ' . $condition . ' limit 1');
		...
		// return $list['logprice'];
		// 2021-6-17 sy
		$list['realpricerate'] = $list['logprice'];
		$list['realprice'] = $list['price'];
		$list['orderprice'] = $list['price'];
	...
	public function getMerchPriceList($merchid, $orderid = 0, $flag = 0, $applyid = 0, $pindex = 0, $psize = 0)
	{
		...
		$condition = ' and u.uniacid=:uniacid and u.id=:merchid and l.status=0 and o.status=3 and o.isparent=0 and o.paytype<>3 ';
		...
		$con = 'o.id,u.merchname,l.id as logid,l.user_fan,l.mch_fan,l.shiwu,l.xiao,l.xiao,l.guan,l.status,u.payrate,o.price,o.goodsprice,o.dispatchprice,discountprice,' . 'o.deductprice,o.deductcredit2,o.isdiscountprice,o.deductenough,o.changeprice,o.agentid,o.seckilldiscountprice,' . 'o.merchdeductenough,o.merchisdiscountprice,o.couponmerchid,o.couponprice,o.couponmerchid,o.ordersn,o.finishtime,o.merchapply,' . 'r.orderprice as rprice,r.applyprice,' . 'comprice.commission_price,comprice.order_id,r.status as rstatus';
		$sql = 'select ' . $con . ' from ' . tablename('ewei_shop_merch_user') . ' u ' . ' left join ' . tablename('ewei_shop_order') . ' o on u.id=o.merchid' . ' left join ' . tablename('ewei_shop_order_refund') . ' r on o.refundid=r.id' . ' left join ' . tablename('ewei_shop_merch_commission_orderprice') . ' comprice on o.id=comprice.order_id left join '.tablename('ewei_fen_log').' l on l.orderid=o.id';
