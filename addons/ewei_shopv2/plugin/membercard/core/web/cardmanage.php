...
class Cardmanage_EweiShopV2Page extends PluginWebPage
{
	public function main()
	{
		...
		header('location: ' . webUrl('membercard/cardmanage/edit', array( 'id' => 11)));
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				$update_data = array('sort_order' => intval($_GPC['sort_order']),'price'=>intval($_GPC['price']),'validate'=>$data['validate'],'discount_rate'=>$_GPC['discount_rate'],'cardmodel'=>$_GPC['cardmodel'], 'stock' => $data['stock'],'name'=>$_GPC['name'],'update_time' => TIMESTAMP, 'status' => intval($_GPC['status']), 'description' => $_GPC['description'], 'goodsids' => $goodsids);
				$goods_update = pdo_update('ewei_shop_member_card', $update_data, array('id' => $id, 'uniacid' => $_W['uniacid']));
				// var_dump(pdo_debug());die;
				if (!$goods_update) {
					show_json(0, '会员卡编辑失败！' . $update_data['price']);
				}
	...
	public function delete()
	{
		...
		//修改到期后 会员表is_vip字段
		pdo_update('ewei_shop_member',array('is_vip'=>0,'level'=>0),array('uid'=>$_W['member']['uid']));
