...
class index_EweiShopV2Page extends PluginMobilePage
{
	...
	public function lottery_info()
	{
		...
		if ($id) {
			...
			$has_changes = $member['pan_num'];
		}
		// var_dump($member);die;
		if(time()>$lottery['end_time'] || time()<$lottery['start_time']){
			$time = 0;
		}else{
			$time = 1;
		}
	...
	public function getreward()
	{
		...
		// var_dump($member['credit2']);die;
		$prr = $member['pan_num']-1;
		pdo_update('ewei_shop_member',array('pan_num'=>$prr),array('uid'=>$member['uid']));
		$data = array('status' => 1, 'id' => $reward_id, 'info' => $reward_info, 'is_reward' => $is_reward,'credit2'=>$prr);
		echo json_encode($data);
		exit();
	}
