	...
	public function getInfo()
	{
		...
		if ($urlData[0] == 'goodsids' || $urlData[0] == 'category' || $urlData[0] == 'groups') {
			...
			if ($urlData[0] == 'category') {
				$pcate = $urlValue[0];
				$goodsql = 'SELECT id,displayorder,title,subtitle,thumb,marketprice,productprice,isdiscount,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,hascommission,commission1_pay,commission,total,description,bargain,nocommission,`type`,ispresell,`virtual`,hasoption,video,buylevels,buygroups,checked FROM ' . tablename('ewei_shop_goods') . ' WHERE FIND_IN_SET(' . $pcate . ',cates) AND status > 0 AND deleted = 0 AND checked=0 AND uniacid =' . $_W['uniacid'] . ' order by displayorder desc,id desc limit 0,' . $goodsnum;
				...
				$goodsql = 'SELECT id,displayorder,title,subtitle,thumb,marketprice,productprice,isdiscount,minprice,maxprice,isdiscount,hascommission,nocommission,commission,commission1_rate,marketprice,commission1_pay,maxprice,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video,buylevels,buygroups FROM ' . tablename('ewei_shop_goods') . ' WHERE id in(' . $goodsid . ') AND status > 0 AND deleted = 0 AND uniacid =' . $_W['uniacid'] . ' limit 0,' . $goodsnum;
				...
				if ($urlData[0] == 'goodsids') {
					...
					if (!empty($goodsids)) {
						...
						$sql = 'SELECT id,displayorder,title,subtitle,thumb,marketprice,productprice,isdiscount,minprice,maxprice,hascommission,nocommission,commission,commission1_rate,marketprice,commission,commission1_pay,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video,buylevels,buygroups FROM ' . tablename('ewei_shop_goods') . ' WHERE id in(' . $goodsid . ') AND uniacid =' . $_W['uniacid'] . ' limit 0,' . $goodsnum;
