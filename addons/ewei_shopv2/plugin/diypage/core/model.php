			...
			if ($mobile && !empty($page['data']['items']) && is_array($page['data']['items'])) {
				foreach ($page['data']['items'] as $itemid => &$item) {
					if ($item['id'] == 'goods') {
							...
							if ($item['params']['goodsdata'] == '2' && empty($item['params']['goodstype'])) {
								...
								if (2 < $item['params']['goodsdata']) {
									...
									if (empty($item['params']['goodstype'])) {
										...
										$goods = pdo_fetchall('select id, title,isdiscount,isdiscount_time, thumb, price as productprice, money as minprice, credit, total, showlevels,showgroups, `type`, goodstype from ' . tablename('ewei_shop_creditshop_goods') . (' where 1 ' . $condition . ' order by ' . $args['order'] . ' limit ') . $args['pagesize'], $params);
									}
									...
									if (!empty($goods) && is_array($goods)) {
										...
										foreach ($goods as $index => $good) {
											...
											if (!empty($showgoods)) {
												...
												//2021-7-19 sy
												// if(time()<$good['isdiscount_time']){
													$item['data'][$childid] = array('thumb' => $good['thumb'], 'title' => $good['title'], 'subtitle' => $good['subtitle'], 'price' => $good['minprice'], 'isdiscount'=>$good['isdiscount'], 'isdiscount_time'=>date('Y-m-d H:i:s',$good['isdiscount_time']),'gid' => $good['id'], 'total' => $good['total'], 'bargain' => $good['bargain'], 'productprice' => $good['productprice'], 'credit' => $good['credit'], 'seecommission' => $good['seecommission'], 'cansee' => $good['cansee'], 'seetitle' => $good['seetitle'], 'ctype' => $good['type'], 'gtype' => $good['goodstype'], 'sales' => $good['sales'] + intval($good['salesreal']));
												// }else{
												// 	$item['data'][$childid] = array('thumb' => $good['thumb'], 'title' => $good['title'], 'subtitle' => $good['subtitle'], 'price' => $good['minprice'], 'isdiscount'=>1, 'isdiscount_time'=>date('Y-m-d H:i:s',$good['isdiscount_time']),'gid' => $good['id'], 'total' => $good['total'], 'bargain' => $good['bargain'], 'productprice' => $good['productprice'], 'credit' => $good['credit'], 'seecommission' => $good['seecommission'], 'cansee' => $good['cansee'], 'seetitle' => $good['seetitle'], 'ctype' => $good['type'], 'gtype' => $good['goodstype'], 'sales' => $good['sales'] + intval($good['salesreal']));
												// }
						...
						$item['info'] = array('avatar' => $member['avatar'], 'nickname' => $member['nickname'], 'levelname' => $level['levelname'], 'textmoney' => $_W['shopset']['trade']['moneytext'], 'textcredit' => $_W['shopset']['trade']['credittext'], 'money' => $member['credit2'],'te_num'=>$member['te_num'],'tong_num'=>$member['tong_num'], 'credit' => intval($member['credit1']), 'membercard' => count($list_membercard['list']));
					}
	...
	public function calculate($str = NULL, $pagetype = false)
	{
		...
		if (strexists($str, 'r=order') || strexists($str, 'r=member.cart') || strexists($str, 'r=coupon.my')) {
			...
			if (strexists($str, 'status=1')) {
				...
				$ordernum['status1'] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_order') . ' where ' . $condition . ' and openid=:openid and uniacid=:uniacid and isparent=0 limit 1', $params);
			}
			...
			if (strexists($str, 'status=2')) {
				...
				$ordernum['status2'] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_order') . ' where ' . $condition . ' and openid=:openid and uniacid=:uniacid and isparent=0 limit 1', $params);
			}
			...
			if (strexists($str, 'status=4')) {
				...
				$ordernum['status4'] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_order') . ' where ' . $condition . ' and openid=:openid and uniacid=:uniacid and isparent=0 limit 1', $params);
			}
