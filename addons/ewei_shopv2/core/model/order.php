...
class Order_EweiShopV2Model
{
		...
		if ($params['from'] == 'return') {
			...
			if ($order['istrade'] == 0) {
				if ($order['status'] == 0) {
					...
					if ($order['isparent'] == 1) {
						// echo 123123;die;
						return $this->setChildOrderPayResult($order, $time, 1);
					}else{

						//不存在子订单情况直接计算分成  2021-6-15  sy
						$goods_order = pdo_getall('ewei_shop_order_goods',array('orderid'=>$order['id']));
						foreach($goods_order as $ke=>$va){

							$goodsmoney = pdo_get('ewei_mch_money',array('type'=>2,'in_id'=>$va['goodsid']));
							pdo_query('update ims_ewei_shop_member set tong_num=tong_num+'.$goodsmoney['tong_num'].',te_num=te_num+'.$goodsmoney['te_num'].' where openid="'.$order['openid'].'"');
							$member = pdo_get('ewei_shop_member',array('openid'=>$order['openid']));

							//大转盘次数
							$zhuan_num = $member['pan_num']+$goodsmoney['zhuan_num'];
							pdo_update('ewei_shop_member',array('pan_num'=>$zhuan_num),array('uid'=>$member['uid']));
							//用户返利
							$membermoney = $goodsmoney['user_fan']*$order['price']/100;
							pdo_query('update ims_mc_members set credit2=credit2+'.$membermoney.' where uid="'.$member['uid'].'"');
							//实物奖池扣除率
							$shiwu = $goodsmoney['shi_num']*$order['price']/100;
							pdo_query('update ims_ewei_game_setting set ok_money=ok_money+'.$shiwu.' where type=1');
							//你消我买扣除率
							$xiao = $goodsmoney['xiao']*$order['price']/100;
							pdo_query('update ims_ewei_game_setting set ok_money=ok_money+'.$xiao.' where type=2');
							//关联售卖扣除率
							$guan = $goodsmoney['guan']*$order['price']/100;
							pdo_query('update ims_ewei_game_setting set ok_money=ok_money+'.$guan.' where type=3');
							
							$log['orderid'] = $order['id'];
							$log['user_fan'] = $membermoney;
							$log['guan'] = $guan;
							$log['shiwu'] = $shiwu;
							$log['xiao'] = $xiao;
							// $log['mch_fan'] = $order['price']-$membermoney-$guan-$shiwu-$xiao;
							$log['mch_fan'] = $goodsmoney['mch_money']*$order['price']/100;
							pdo_insert('ewei_fen_log',$log);
	...
	public function setChildOrderPayResult($order, $time, $type)
	{
		...
		if (!empty($list)) {
			...
			$change_data['remark'] = 6666;

			foreach ($list as $k => $v) {
				if ($v['status'] == 0) {
					pdo_update('ewei_shop_order', $change_data, array('id' => $v['id']));
					//子订单在此进行分成 2021-6-15 sy

					$goods_order = pdo_get('ewei_shop_order_goods',array('orderid'=>$v['id']));

					$goodsmoney = pdo_get('ewei_mch_money',array('type'=>2,'in_id'=>$goods_order['goodsid']));
					pdo_query('update ims_ewei_shop_member set tong_num=tong_num+'.$goodsmoney['tong_num'].',te_num=te_num+'.$goodsmoney['te_num'].' where openid="'.$order['openid'].'"');
					$member = pdo_get('ewei_shop_member',array('openid'=>$order['openid']));


					//大转盘次数
					pdo_update('ewei_shop_member',array('pan_num'=>$member['pan_num']+1),array('uid'=>$member['uid']));
					//用户返利
					$membermoney = $goodsmoney['user_fan']*$v['price']/100;
					pdo_query('update ims_mc_members set credit2=credit2+'.$membermoney.' where uid="'.$member['uid'].'"');
					//实物奖池扣除率
					$shiwu = $goodsmoney['shi_num']*$v['price']/100;
					pdo_query('update ims_ewei_game_setting set ok_money=ok_money+'.$shiwu.' where type=1');
					//你消我买扣除率
					$xiao = $goodsmoney['xiao']*$v['price']/100;
					pdo_query('update ims_ewei_game_setting set ok_money=ok_money+'.$xiao.' where type=2');
					//关联售卖扣除率
					$guan = $goodsmoney['guan']*$v['price']/100;
					pdo_query('update ims_ewei_game_setting set ok_money=ok_money+'.$guan.' where type=3');
					...
					$log['shiwu'] = $shiwu;
					$log['xiao'] = $xiao;
					$log['mch_fan'] = $v['price']-$membermoney-$guan-$shiwu-$xiao;
					pdo_insert('ewei_fen_log',$log);
	...
	{
		global $_W;
		$list = pdo_fetchall('select id,ordersn,price,status,finishtime,couponid,merchid  from ' . tablename('ewei_shop_order') . ' where  parentid=:parentid and uniacid=:uniacid', array(':parentid' => $orderid, ':uniacid' => $_W['uniacid']));
		return $list;
	}
	...
	public function getChildOrderPrice(&$order, &$goods, &$dispatch_array, $merch_array, $sale_plugin, $discountprice_array, $orderid = 0)
	{
		...
		$member = pdo_get('ewei_shop_member',array('openid'=>$order['openid']));
		$vip = pdo_get('ewei_shop_member_card',array('id'=>11));
		...
		foreach ($ch_order as $k => $v) {
			...
			if($member['is_vip']==1){
				$price = $price*$vip['discount_rate']/10;
			}
