...
class Rank_EweiShopV2Page extends MobileLoginPage
{
	...
	public function order_rank()
	{
		...
		$member = pdo_fetchall("select u.rowNo from (select uid,(@rowNum:=@rowNum+1) as rowNo from ims_ewei_shop_member,(select (@rowNum :=0) ) b order by win_num desc ) u where u.uid=".$user['uid']);
	...
	public function ajaxorderpage()
	{
		...
		$sql = 'SELECT nickname,avatar,uid,win_num from  ' . tablename('ewei_shop_member') .' order by win_num desc'. $limit;

		$result = pdo_fetchall($sql, array(':uniacid' => $_W['uniacid']));

		// foreach($result as $k=>$v){
		// 	$num = pdo_fetchall('select count(*) from '.tablename('ewei_game_log').' where status=1 and member_id='.$v['uid']);
		// 	$result[$k]['num'] = $num['0']['count(*)'];
		// }

		// foreach ($result as $key => $row) {
		//     $distance[$key] = $row['num'];
		// }
		// array_multisort($distance, SORT_DESC, $result);
		// foreach($result as $ke=>$va){
		// 	if($va['uid']==$_W['member']['uid']){
		// 		$pai = $ke+1;
		// 	}
		// }
