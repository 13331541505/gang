...
class Api_EweiShopV2Page extends ComWebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			// return false;
			$data = array('uniacid' => $_W['uniacid'], 'game_name' => trim($_GPC['game_name']), 'game_img' => trim($_GPC['game_img']), 'content' => trim($_GPC['content']), 'is_cash' => in_array(1,$_GPC['is_cash'])?1:0,'is_tong' => in_array(2,$_GPC['is_cash'])?1:0,'is_te' => $_GPC['is_te'],'is_te' => in_array(3,$_GPC['is_cash'])?1:0, 'member_max' => trim($_GPC['member_max']), 'member_min' => trim($_GPC['member_min']), 'game_money' => trim($_GPC['game_money']), 'money_min' => $_GPC['money_min'], 'start_time' => $_GPC['start_time'], 'time_for' => $_GPC['time_for'], 'hexiao' => trim($_GPC['hexiao']), 'ok_where' => json_encode($_GPC['ok_where']),'goods_id'=>json_encode($_GPC['goodsids']));
