...
class Printer_EweiShopV2ComModel extends ComModel
{
	...
	{
		return array(self::PRINTRT_365_S1 => '365云打印(编号kdt1)', self::PRINTRT_365 => '365云打印(编号kdt2)', self::PRINTRT_YILIANYUN => '微米云K1、K2、K3', self::PRINTRT_YILIANYUN_NEW => '微米云K2S、K3S、K4、M1', self::PRINTRT_YILIANYUN_AUTH2 => '微米云auth2.0接口(支持K4及以上)', self::PRINTRT_FEIE => '飞鹅打印机', self::PRINTRT_FEIE_NEW => '飞鹅打印机(新接口)');
	}
...
剩余次数:' . ($verify['lastverifys'] - $verify['times']) . '次';
	...
	/**
     * 微米云 auth2.0新接口获取token
     * @param $client_id  微米云颁发给开发者的应用ID
     * @param $client_secret 微米云颁发给开发者的应用密钥
     * @param $merchid 商户id
