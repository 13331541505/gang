...
class Index_EweiShopV2Page extends ComWebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			$usercount = pdo_fetch('select * from ' . tablename('ewei_shop_merch_account') . (' where ' . $where . ' limit 1'), $params);
			
			$merchinfo = pdo_get('ewei_shop_merch_user',array('store_id'=>$id));

			if ($usercount && $merchinfo['id']!=$usercount['merchid']) {
				show_json(0, '账号名已存在');
			}
			...
			$mch['uniacid']=  $_W['uniacid'];
			$mch['merchname']=  trim($_GPC['storename']);
			$mch['salecate']=  trim($_GPC['desc']);
			$mch['desc']=  trim($_GPC['desc']);
			$mch['realname']=  trim($_GPC['realname']);
			$mch['mobile']=  trim($_GPC['mobile']);
			$mch['status']=  1;
			$mch['accounttime']=  time();
			$mch['logo']=  save_media($_GPC['logo']);
			$mch['address']=  trim($_GPC['address']);
			$mch['lat']=  $_GPC['map']['lat'];
			$mch['lng']=  $_GPC['map']['lng'];
			$mch['is_recommand']=  1;
			$mch['openid']=  "";
			$mch['pluginset']=  'a:5:{s:10:"creditshop";a:1:{s:5:"close";s:1:"0";}s:5:"quick";a:1:{s:5:"close";s:1:"0";}s:6:"taobao";a:1:{s:5:"close";s:1:"0";}s:8:"exhelper";a:1:{s:5:"close";s:1:"0";}s:7:"diypage";a:1:{s:5:"close";s:1:"0";}}';
			...
			$data1['tong_num'] = $_GPC['tong_num'];
			$data1['te_num'] = $_GPC['te_num'];
			$data1['user_fan'] = $_GPC['user_fan'];
			$data1['shi_num'] = $_GPC['shi_num'];
			$data1['xiao'] = $_GPC['xiao'];
			$data1['guan'] = $_GPC['guan'];
			$data1['mch_money'] = $_GPC['mch_money'];
			$data1['type'] = 1;

			$salt = random(8);
			$pwd = md5(trim($_GPC['pwd']) . $salt);
			$account = array('uniacid' => $_W['uniacid'],'username' => $username, 'pwd' => $pwd, 'salt' => $salt, 'status' => 1, 'perms' => serialize(array()), 'isfounder' => 1);

			if (!empty($id)) {
				pdo_update('ewei_shop_store', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));

				pdo_update('ewei_shop_merch_user', $mch, array('store_id' => $id, 'uniacid' => $_W['uniacid']));
				$merchinfo = pdo_get('ewei_shop_merch_user',array('store_id'=>$id));

				pdo_update('ewei_mch_money', $data1, array('in_id' => $merchinfo['id'], 'type' =>1));
				if(!empty($_GPC['pwd'])){
					$account['pwd'] = $pwd;
				}
				pdo_update('ewei_shop_merch_account',$account,array('merchid'=>$merchinfo['id']));
				...
				$id = pdo_insertid();
				$mch['store_id'] = $id;
				$ree = pdo_insert('ewei_shop_merch_user',$mch);
				$merch_id = pdo_insertid();
				$account['merchid'] = $merch_id;
				
				pdo_insert('ewei_shop_merch_account', $account);

				$data1['in_id'] = $merch_id;
				pdo_insert('ewei_mch_money', $data1);
				
				$url = mobileUrl('shop/paycode', ['shop_id'=>$merch_id], true);
	    		$qrcode = m('qrcode')->createQrcode($url);
	    		pdo_update('ewei_shop_merch_user', array('pay_code'=>$qrcode), array('id' => $merch_id));
		...
		$merchinfo = pdo_get('ewei_shop_merch_user',array('store_id'=>$id));
		$gamesetting = pdo_get('ewei_mch_money',array('type'=>1,'in_id'=>$merchinfo['id']));
		$account = pdo_get('ewei_shop_merch_account',array('merchid'=>$merchinfo['id']));
