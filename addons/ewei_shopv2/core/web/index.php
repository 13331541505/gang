...
class Index_EweiShopV2Page extends WebPage 
{
	public function main()
	{
		...
		$shiwu = pdo_fetchall('select sum(shiwu) from ims_ewei_fen_log where status = 0');
		$shiwu = $shiwu[0]['sum(shiwu)'];

		$xiao = pdo_fetchall('select sum(xiao) from ims_ewei_fen_log where status = 0');
		$xiao = $xiao[0]['sum(xiao)'];

		$guan = pdo_fetchall('select sum(guan) from ims_ewei_fen_log where status = 0');
		$guan = $guan[0]['sum(guan)'];
