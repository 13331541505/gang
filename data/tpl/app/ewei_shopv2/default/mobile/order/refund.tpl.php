        ...
        <div class="fui-cell">
            ...
            <div class="fui-cell-info">
                ...
                <select id="rtype">
                    <?php  if($canreturns == 1) { ?>
                    <!--当产品金额被余额全部抵扣之后.允许换货-->
                    <?php  if(($refundgoods['exchange'] && $order['cannotrefund']) || !$order['cannotrefund']) { ?>
                    <option value="2" <?php  if($refund['rtype']=='2') { ?>selected<?php  } ?>>换货</option>
                    <?php  } ?>
                    <?php  } else { ?>
                        <?php  if(($refundgoods['refund'] > 0 && $order['cannotrefund']) || !$order['cannotrefund']) { ?>
                        <option value="0" <?php  if($refund['rtype']=='0') { ?>selected<?php  } ?>>退款(仅退款不退货)</option>
                        <?php  } ?>
                        <?php  if($order['status']==2 || $order['status']==3) { ?>
                        <?php  if(($refundgoods['returngoods'] > 0 && $order['cannotrefund']) || !$order['cannotrefund']) { ?>
                        <option value="1" <?php  if($refund['rtype']=='1') { ?>selected<?php  } ?>>退货退款</option>
                        <?php  } ?>
                        <?php  if(($refundgoods['exchange'] > 0 && $order['cannotrefund']) || !$order['cannotrefund']) { ?>
                            <option value="2" <?php  if($refund['rtype']=='2') { ?>selected<?php  } ?>>换货</option>
                        <?php  } ?>
                        <?php  } ?>
                    <?php  } ?>
