				...
				<div class="buybtn" data-cartid="<?php  echo $row['id'];?>" >续费</div>
					...
					<a class='cardindex content <?php  echo $row['card_style'];?>'  data-index=<?php  echo $i;?>  href="<?php  echo mobileUrl('membercard/detail',array('id'=>$row['id'],'k'=>$i,'type'=>all))?>">
						...
						<div class='content-inner'>
							...
							<div class="equity"><?php  if($row['cardmodel'] == 2) { ?>指定商品·<?php  } else { ?>全部商品·<?php  } ?><?php  if($row['shipping']) { ?>免费包邮·<?php  } ?><?php  if($row['discount_rate']) { ?><?php  echo $row['discount_rate'];?>折优惠·<?php  } ?><?php  if($row['is_card_coupon']) { ?>开卡送券·<?php  } ?><?php  if($row['is_month_coupon']) { ?>每月领券·<?php  } ?>更多特权</div>
						</div>
					</a>
				</div>
				<?php  } } ?>
