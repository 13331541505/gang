	...
	<div class="fui-content navbar" style="bottom: 0">
		...
		<script id='tpl_order_index_list' type='text/html'>
			...
			<div class='card-group'>
				...
				<div class='card-subtitle'>会员开卡即送<%if list.coupon_card.length>0%>优惠券<%/if%><%if list.coupon_card.length>0&&list.is_card_points>0 %>、<%/if%><%if list.is_card_points>0%><?php echo empty($data['credittext'])?'积分':$data['credittext']?><%/if%></div>
				...
				<div class='coupon-list left'>
					...
					<div class='coupon-list-item'>
						...
						<div class='coupon-inner'>
							...
							<div>购物返<span><%kai_card.backcredit%></span><?php echo empty($data['credittext'])?'积分':$data['credittext']?></div>
							<%/if%>
				...
				<div class='card-fiche golden' style='margin-top: 0;'>
					...
					<div class='fiche-inner'>
						<div class='title'>开通立享<?php echo empty($data['credittext'])?'积分':$data['credittext']?></div>
						<div class='subtitle'><%list.card_points%>
							<span><?php echo empty($data['credittext'])?'积分':$data['credittext']?></span>
						</div>
					</div>
					<div class='fiche-btn'>
						<%if list.kaika_jifen%>
						...
						<%else%>
						<div class='btn-submit border'>开通送<?php echo empty($data['credittext'])?'积分':$data['credittext']?></div>
						<%/if%>
					</div>
				</div>
				<%/if%>
			...
			<div class='card-group' style='padding-bottom: 0.85rem;'>
				...
				<div class='card-subtitle'>会员可每月领取<%if list.coupon_month.length>0%>优惠券<%/if%><%if list.coupon_month.length>0&&list.is_month_points>0%>、<%/if%><%if list.is_month_points>0%><?php echo empty($data['credittext'])?'积分':$data['credittext']?><%/if%></div>
				...
				<div class='coupon-list left'>
					...
					<div class='coupon-list-item'>
						...
						<div class='coupon-inner'>
							...
							<div>购物返<span><%kai_month_card.backcredit%></span><?php echo empty($data['credittext'])?'积分':$data['credittext']?></div>
							<%/if%>
							...
							<%/if%>
							<div class='condition'><%kai_month_card.couponname%></div>
						</div>
					</div>
				...
				<div class='card-fiche golden' style='margin-top: 0;'>
					...
					<div class='fiche-inner'>
						<div class='title'>会员每月领取<?php echo empty($data['credittext'])?'积分':$data['credittext']?></div>
					...
					<div class='fiche-btn'>
						<%if list.goumai==1%>
						<div class='btn-submit border'>每月领<?php echo empty($data['credittext'])?'积分':$data['credittext']?></div>
						<%else%>
						<%if list.is_check_month_point%>
			...
			<div class='card-group' style='padding-bottom: 0.85rem;'>
				<div class='card-title'>每月领取</div>
				<div class='card-subtitle'>会员可每月领取<%if list.coupon_month.length>0%>优惠券<%/if%><%if list.is_month_points>0&&list.coupon_month.length>0%>、<%/if%><%if list.is_month_points>0%><?php echo empty($data['credittext'])?'积分':$data['credittext']?><%/if%></div>
				<%if list.coupon_month.length>0%>
				<div class='coupon-list left'>
					<%each list.coupon_month as kai_month_card%>
					...
					<div class='coupon-list-item already'>
						...
						<div class='coupon-inner'>
							<div class='coupon-inner-media'>
								...
								<%/if%>
								<%if kai_card.backcredit>0%>
								<div class='price'>购物返<span><%kai_card.backcredit%></span><?php echo empty($data['credittext'])?'积分':$data['credittext']?></div>
								<%/if%>
								<%if kai_card.backredpack>0%>
